...
ETH_DMADescTypeDef  DMARxDscrTab[ETH_RXBUFNB] __attribute__((section(".sram2_non_cached_device"),zero_init)); /* Ethernet Rx DMA Descriptor */
ETH_DMADescTypeDef  DMATxDscrTab[ETH_TXBUFNB] __attribute__((section(".sram2_non_cached_device"),zero_init)); /* Ethernet Tx DMA Descriptor */
uint8_t Rx_Buff[ETH_RXBUFNB][ETH_RX_BUF_SIZE] __attribute__((section(".sram2_non_cached_normal"),zero_init)); /* Ethernet Receive Buffer */
uint8_t Tx_Buff[ETH_TXBUFNB][ETH_TX_BUF_SIZE] __attribute__((section(".sram2_non_cached_normal"),zero_init)); /* Ethernet Transmit Buffer */
...
{
  bufferin++;
  if (event_callback != NULL) (*event_callback)();
}
...
void ethernetif_low_deinit()
{
  ...
  for(int i=0;i<ETH_TXBUFNB;i++)
  {
    ...
    {
      DMATxDscrTab[i].Buffer1Addr = 0;
      if (*buffer_output_callback) (*buffer_output_callback)((uint8_t*) DMATxDscrTab[i].Buffer1Addr);
    }
  }
}
...
int16_t ethernetif_low_input(uint8_t *payload,uint16_t len)
{
  ...
  n = EthHandle.RxFrameInfos.length;
  dmarxdesc = EthHandle.RxFrameInfos.FSRxDesc;

  if ( (payload == NULL) || (len < n) )
  {
    NET_WARNING("LwIP pool is full or buffer is too small. Dropping an Ethernet RX frame of size %lu.\n", n);
  }
  else
  {
#if !defined(USE_CACHED_ETH_BUFFERS)
    SCB_InvalidateDCache_by_Addr ((uint32_t *)dmarxdesc->Buffer1Addr ,n );
#endif
    memcpy(payload,(uint8_t*) dmarxdesc->Buffer1Addr, n);
  }

  /* Clear Segment_Count */
  NET_ASSERT(RxFrameInfos.SegCount == 1," multiple segments %lu\n", EthHandle.RxFrameInfos.SegCount);

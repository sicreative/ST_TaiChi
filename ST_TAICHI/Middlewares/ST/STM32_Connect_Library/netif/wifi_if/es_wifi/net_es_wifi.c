...
#define WIFI_FREE_SOCKET                 0
#define WIFI_ALLOCATED_SOCKET            1
#define WIFI_BIND_SOCKET                 2
#define WIFI_SEND_OK                     4
#define WIFI_RECV_OK                     8
#define WIFI_CONNECTED_SOCKET            16
#define WIFI_STARTED_CLIENT_SOCKET       32
#define WIFI_STARTED_SERVER_SOCKET       64
...
static int32_t es_wifi_recvfrom (int32_t sock, uint8_t *buf, int32_t len, int32_t flags, sockaddr_t *from, int32_t *fromlen);
...
static int32_t es_wifi_shutdown (int32_t sock, uint32_t mode);

static int32_t es_wifi_gethostbyname(net_if_handle_t *pnetif,sockaddr_t *addr,char *name);
static int32_t es_wifi_ping(net_if_handle_t *pnetif,sockaddr_t *addr,int count , int delay, int response[]);

/* Declaration and definition of class-specific functions */
static void check_connection_lost(net_if_handle_t *pnetif, int n);
...
int32_t es_wifi_if_start(net_if_handle_t * pnetif)
{
  ...
  if (wifi_probe(&pnetif->pdrv->context)== NET_OK)
  {
    ...
    else
    {
      ...
      for (int i = 0; i < ESWIFI_MAX_CHANNEL_NBR; i++)
      {
        WifiChannel[i].status          = WIFI_FREE_SOCKET;
        WifiChannel[i].Number          = i;
        WifiChannel[i].maxpayload      = 1024; //ToDo
...
static int32_t es_wifi_if_connect (net_if_handle_t * pnetif)
{
  ...
  ret = ES_WIFI_Connect(pEsWifiObj,credentials->ssid, credentials->psk, (ES_WIFI_SecurityType_t) credentials->security_mode);
...
#ifdef NET_MBEDTLS_WIFI_MODULE_SUPPORT
    eswifi_tls_data_t *tlsdata
    uint32_t      casize,keysize,devsize;
#endif
    ...
    if (addrlen == sizeof(sockaddr_in))
    {
      ...
      switch(WifiChannel[sock].protocol)
      {
        ...
        case NET_IPPROTO_TCP_TLS :
          ...
          ret = ES_WIFI_StoreCreds( WifiChannel[sock].pnetif->ll_drv_obj, ES_WIFI_FUNCTION_TLS , ES_WIFI_TLS_MULTIPLE_WRITE_SLOT,
...
static int32_t es_wifi_shutdown (int32_t sock, uint32_t mode)
{
  ...
  {
    if (mode == NET_SHUTDOWN_R ) WifiChannel[sock].status &=  ~WIFI_RECV_OK;
    if (mode == NET_SHUTDOWN_W ) WifiChannel[sock].status &=  ~WIFI_SEND_OK;
    if (mode == NET_SHUTDOWN_RW ) WifiChannel[sock].status &=  ~ ( WIFI_SEND_OK | WIFI_RECV_OK);
  }
  return NET_OK;
}
...
static int32_t es_wifi_send (int32_t sock, uint8_t *buf, int32_t len,int32_t flags)
{
  ...
  if (len > ES_WIFI_PAYLOAD_SIZE) len = ES_WIFI_PAYLOAD_SIZE;
...
static int32_t es_wifi_recvfrom (int32_t sock, uint8_t *buf, int32_t len, int32_t flags, sockaddr_t *from, int32_t *fromlen)
{
  ...
  if (!(WifiChannel [sock].status & WIFI_STARTED_CLIENT_SOCKET))
  {
    ...
    uint8_t *buf = NULL;  // If no better idea, could send an mdns advertisement.
    int32_t len = 0;
    flags = 0;
    sockaddr_in saddr;
    saddr.sin_len = sizeof(sockaddr_in);
    saddr.sin_family = NET_AF_INET;
    saddr.sin_addr = net_aton_r("224.0.0.251");
    saddr.sin_port = net_htons(5353);

    ret = es_wifi_sendto (sock, buf, len, flags, (sockaddr_t *) &saddr, sizeof(sockaddr_in));
  }
...
static int32_t es_wifi_setsockopt( int32_t sock, int32_t level, int32_t optname, const void *optvalue, int32_t optlen)
{
  ...
  else
  {
    ...
    case NET_SO_RCVTIMEO:
      /* Receive time out. */
       WifiChannel[sock].recvtimeout = *(int32_t *) optvalue;
      ret = NET_OK;
      break;
...
static int32_t es_wifi_gethostbyname(net_if_handle_t *pnetif, sockaddr_t *addr, char *name)
{
  ...
  if (ES_WIFI_STATUS_OK == ES_WIFI_DNS_LookUp(pnetif->pdrv->context, name, ipaddr))
  {
...
int32_t es_wifi_ping(net_if_handle_t *pnetif,sockaddr_t *addr,int count , int delay, int response[])
{
  int32_t ret;
  uint8_t     ipaddr[6];

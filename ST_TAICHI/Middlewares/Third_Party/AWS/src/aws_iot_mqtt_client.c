...
#if !DISABLE_METRICS
#define SDK_METRICS_LEN 50
#define SDK_METRICS_TEMPLATE "?SDK=C,v%d.%d.%d&Platform=%s"
...
IoT_Error_t aws_iot_mqtt_set_connect_params(AWS_IoT_Client *pClient, IoT_Client_Connect_Params *pNewConnectParams) {
	...
	if (0 == strlen(pUsernameTemp)) {
		snprintf(pUsernameTemp, SDK_METRICS_LEN, SDK_METRICS_TEMPLATE, VERSION_MAJOR, VERSION_MINOR, VERSION_PATCH, SDK_METRICS_PLATFORM_NAME);
	}
